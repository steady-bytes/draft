// Command grpc-call is a Foundry plugin: a small, separate Draft service
// that implements StepExecutor.Execute (see rpc.go) and publishes itself
// to Foundry's PluginCatalogService on startup via the chassis.Effect
...
// unchanged).
//
// What this plugin actually does (rpc.go): makes a unary call to an
// arbitrary external gRPC server, resolving the request/response shape at
// runtime via that server's own reflection service — see rpc.go's file
// comment for why this exists alongside Bench's built-in
// bench://grpc-call@v1.
package main
