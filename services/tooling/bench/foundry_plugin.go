// This file implements Phase 6/11: resolving a `uses: foundry://<name>@<version>`
// step reference to a live plugin instance and calling its StepExecutor RPC,
// per docs/website/content/docs/architecture/foundry-plugin-repository.md's
...
//     "Foundry does not execute plugins itself").
...
//     version of a given plugin at a time (matches Foundry's own
...
// registryLister is the read side of *pgResultStore the foundry:// executor
...
// foundryPluginExecutor is the Executor behind every `foundry://...` reference
...
// foundryPluginExecutor is the same shape: one instance, per-call resolution
...
// queried fresh on every Execute call (see NewFoundryPluginExecutor), so a
// registry added, edited, or removed through the Settings page changes what
// a foundry:// step can resolve against on the very next run, not after a
// restart — the same "dynamic, not cached at construction" fix Phase 10c
// already applied to webhook routing.
type foundryPluginExecutor struct {
	registries registryLister
	plugins    PluginResolver
	httpClient connect.HTTPClient
}

// NewFoundryPluginExecutor builds the foundry:// executor against the current
...
func NewFoundryPluginExecutor(httpClient connect.HTTPClient, registries registryLister, plugins PluginResolver) Executor {
	return &foundryPluginExecutor{
...
func (e *foundryPluginExecutor) Execute(ctx context.Context, step *workflowv1.Step, with *structpb.Struct) (*ExecutionResult, error) {
	name, version, err := parseFoundryUses(step.GetUses())
	if err != nil {
		return nil, err
	}
	...
	if err != nil {
		return nil, fmt.Errorf("foundry plugin %q@%q: failed to list configured plugin registries: %w", name, version, err)
	}
	if len(registries) == 0 {
		return nil, fmt.Errorf("foundry plugin %q@%q: no plugin registries are configured (see the Settings page)", name, version)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("foundry plugin %q@%q: %w", name, version, err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("foundry plugin %q@%q: calling %s: %w", name, version, addr, err)
	}

	requestHeaders := map[string]interface{}{}
	// NewTraceClientInterceptor (see the call above) sets this same header on
	// the actual outbound request whenever ctx carries a span; probed
	// independently here purely for display -- Bench never sees the plugin's
	// own request/response headers beyond this RPC boundary (the plugin's own
	// outbound call, e.g. http-call's to whatever URL the workflow specifies,
	// happens in a separate process Bench has no visibility into).
...
func findPluginInRegistries(ctx context.Context, httpClient connect.HTTPClient, registries []*settingsv1.PluginRegistry, name, version string) (string, error) {
	...
	return "", fmt.Errorf("foundry plugin %q@%q: not published to any configured registry (checked: %s)", name, version, strings.Join(checked, ", "))
}

// parseFoundryUses splits "foundry://name@version" into its name and version.
func parseFoundryUses(uses string) (name, version string, err error) {
	...
	if !ok || name == "" || version == "" {
		return "", "", fmt.Errorf("step uses %q: expected foundry://<name>@<version>", uses)
	}
