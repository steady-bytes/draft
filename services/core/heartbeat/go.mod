module github.com/steady-bytes/draft/services/core/heartbeat

go 1.25.3

replace github.com/steady-bytes/draft/api => ../../../api

// Local chassis is used (not a published version) because this service exists to
// exercise the effect-tracking primitive added to chassis directly — see
// pkg/chassis/effect.go and docs/website/content/docs/architecture/
// chassis-composability.md.
replace github.com/steady-bytes/draft/pkg/chassis => ../../../pkg/chassis

// Local chassis's Logger interface (WithCallDepth) is ahead of the last published
// pkg/loggers tag, so this also has to point at the local copy to satisfy it.
