...
steps:
  ...
  - name: notify-slack-on-seed-data
    uses: foundry://slack-notify@v2      # a plugin resolved through Foundry + Blueprint
    with:
      channel: "#e2e-results"
...
- **`uses:`** is a plugin reference, `scheme://name@version`. `bench://` addresses executors built into Bench itself (see below); `foundry://` addresses a plugin resolved through the Foundry catalog. This mirrors GitHub Actions' `uses: owner/action@version` and Drone's `image: plugins/name` — the same shape three independent systems converged on for "delegate this step to a named, versioned, external unit."
...
## Built-in executors vs. Foundry plugins

The single most common step — "make a gRPC call, check the response" — shouldn't require standing up a separate registered service just to run a test. Bench ships a small set of built-in executors compiled directly into it (`bench://grpc-call@v1` above; a `bench://http-call@v1` for testing anything fronted by Fuse over plain HTTP would be the other obvious one). Anything more specific — posting to Slack, seeding a database with fixture data, polling an external system — is a Foundry plugin: a separately deployed, separately versioned, separately owned service. This mirrors Drone's split between its handful of built-in step kinds and its community plugin index, and keeps Bench itself from accumulating integrations for every system a test might ever need to touch.
...
| Settings | Add, edit, or remove the foundry-compatible plugin registries the search sidebar and `foundry://` steps resolve against, seeded from `config.yaml` at startup | `table` for the registry list; `input`/`alert` for a failed connection check |
...
Authoring a `foundry://` step today means knowing a plugin's exact name, version, and `with:` shape ahead of time — in practice, browsing to Foundry's own catalog UI in a separate tab, reading its detail page, and hand-copying the `uses:` snippet it renders there. And *which* registry a `foundry://` step even resolves against is a single `foundry.address` value in `config.yaml`, read once at startup — no way to point Bench at a different (or additional) foundry-compatible registry without editing a file and restarting. This section designs three things closing that gap: **plugin registries declared in `config.yaml`** and loaded into the running service, a **Registries page** to add, edit, or remove them at runtime without a restart, and a **search sidebar** on the New/Edit workflow page that turns "browse Foundry, copy a snippet, switch tabs, paste it" into one page. Phase 11 in the [Implementation plan](#implementation-plan) tracks building it; nothing in this section is built yet.

**"Foundry-compatible" is already true, not a new requirement.** `foundry_plugin.go`'s executor has only ever depended on the `tooling.plugin_catalog.v1.PluginCatalogService` proto contract — never anything Foundry-specific — so "connect to a foundry-compatible registry" doesn't change what Bench assumes about the other end, only that the set of addresses it checks is now configurable and plural instead of fixed and singular.
...
**Live-checked, not blindly saved.** `AddPluginRegistry`/`UpdatePluginRegistry` make a real `PluginCatalogService.List` call (`page_size: 1`) against the submitted address before persisting it, and fail clearly if nothing foundry-compatible answers there — the same "reject bad input at the boundary" discipline `ParseWorkflow` already applies to workflow YAML, rather than silently saving an address that then breaks every search and every `foundry://` step that happens to resolve against it.
...
**Dynamic, like everything else this doc's "no restart" thread already insists on.** `foundryPluginExecutor` currently builds one `PluginCatalogServiceClient` at `Scheduler` construction time, pointed at whatever `foundry.address` said at process startup. That has to change to resolve the *current* list of registries fresh on every `Execute` call — otherwise adding, editing, or removing a registry through the UI wouldn't actually change what a `foundry://` step can resolve against without a restart, exactly the gap Phase 10c closed for webhook routing. Same principle, same fix shape, now over a list instead of one address.

**The search sidebar needs no new Bench-side RPC for the search itself — Bench is just another client of an API that already exists, N times over instead of once.** New/Edit workflow's layout gains a right-hand panel: a search input and a scrollable result list, calling Foundry's existing `Search`/`List` RPCs directly from `ui.go` against every currently-configured registry, the same way `foundryPluginExecutor` already does for execution. htmx handles the fetch — a debounced `hx-trigger` on the search input swaps a results fragment — no new client-side framework. Each result card shows a small badge naming which registry it came from.
...
Proto definitions (`api/tooling/workflow/v1`, `api/tooling/step_executor/v1`) are Phase 0 of the [cross-cutting plan](/docs/architecture/e2e-testing-overview#implementation-plan) and a prerequisite for everything below. Each phase here produces something runnable and testable before the next one starts — none of Phases 1–5 need Foundry to exist. See the [tracking convention](/docs/architecture/e2e-testing-overview#implementation-plan) for how checked items are annotated.
...
- [x] **11. Plugin registry settings + a step-search sidebar.** Design in [Connecting a plugin registry](#connecting-a-plugin-registry) above. Deliverable: registries declared in `config.yaml` are seeded at startup; adding, editing, or removing one on the Settings page is live-checked, saved, and immediately changes what `foundry://` steps and the search sidebar can see — no restart; the New/Edit workflow page's sidebar lets a real plugin from any configured registry be searched and appended into the textarea as a correctly-formed step, no manual copy-paste from Foundry's own UI required anymore. _(completed 2026-08-23 07:39 UTC — all five sub-phases built and live-verified together, not separately: added a second registry through the real Settings form (live-checked against real running Foundry), confirmed the search sidebar federated both registries' catalogs (`slack-notify`/`catalyst-consume`, each tagged by source), re-ran `catalyst-consume-e2e` and confirmed `foundry://` resolution still passed with `Detail.registry` now naming which registry matched, and confirmed seed-once on a real restart (`seeded_this_run=0` for both the pre-existing workflow and registry rows). 69 tests pass (`-race` clean, up from 61), including a fake-registry-backed suite (`registry_write_test.go`, `foundry_plugin_test.go`) that directly proves first-match-wins federation and the documented silent-collision behavior, not just the pure-Go helpers in isolation.)_
  ...
  - [x] **11b. Config seeding.** New `bench.plugin_registries` config key (a list of `{name, address}`); `plugin_registries.go`'s `loadPluginRegistries` mirrors `loadWorkflowsDir`'s seed-once behavior exactly. Unset falls back to one implicit `default` registry from the existing static `foundry.address` value — verified live: the real running stack (never given a `bench.plugin_registries` config) seeded `default` → `http://localhost:9301` (Foundry's real address) with zero config changes needed.
...
- **No version grouping in search results.** `Search`/`List` return every published version of a plugin as its own row, matching Foundry's own catalog page today, rather than one row per name with a version picker. Fine for a catalog that stays small; worth revisiting if it grows.
