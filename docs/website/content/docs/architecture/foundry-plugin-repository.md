---
weight: 32
title: 'Foundry — Plugin Repository'
...
Foundry is the plugin repository described in [End-to-End Testing — Overview](/docs/architecture/e2e-testing-overview) — the catalog [Bench](/docs/architecture/bench-workflow-engine) resolves a step's `uses: foundry://name@version` reference against. This document covers what a plugin actually is, the catalog it lives in, how a plugin publishes itself, how Bench resolves and calls one, and Foundry's own UI.
...
Foundry doesn't run plugins — it's metadata about them. A plugin publishes a manifest describing itself; Foundry stores and serves that manifest; Bench queries it to resolve a `uses:` reference and to validate a step's `with:` block before ever calling the plugin.

```yaml
# published by the plugin at startup, or via foundry-cli publish
...
// publishing to Foundry's catalog is exactly the "ad hoc effect with an inverse" shape
...
c.Effect("foundry-catalog-entry", func() (func(context.Context) error, error) {
    if err := publishToFoundry(manifest); err != nil {
        return nil, err
    }
    return retractFromFoundry, nil
})

defer c.Start()
```

This is the same pattern this session already landed in Chassis for [`auth`'s service-address registration](/docs/architecture/chassis-composability#worked-example-giving-auth-a-real-inverse) — a plugin's presence in Foundry's catalog is exactly the kind of ad hoc effect `chassis.Effect` exists for, tracked so it's automatically retracted on graceful shutdown instead of left stale.

Two separate registrations, two separate purposes: **Blueprint** answers "where is a live instance of `slack-notify` right now" (ordinary service discovery, the same as any RPC dependency); **Foundry** answers "what versions of `slack-notify` exist, and what does `v2`'s config look like" (catalog metadata, versioned independently of any particular running instance). Bench needs both to resolve a step: Foundry to validate the reference and its config, Blueprint to find somewhere to actually send the `Execute` call.

## The UI

Foundry's UI shares Bench's stack and DaisyUI theme (`templ` + htmx + DaisyUI, or `html/template` as the zero-dependency fallback — see [Bench's UI section](/docs/architecture/bench-workflow-engine#the-ui) for the rationale), since a developer moving between "which plugins are available" and "what did my last test run do" should not feel like they've left one product for another.
...
- **Version compatibility**: today a workflow pins an exact version (`slack-notify@v2`); whether Foundry should support range constraints (`^v2`) the way a package manager would is deliberately left open — it's a nominal-vs-structural-linking tension, the same shape as resolving a dependency by name alone versus by verified interface compatibility, and probably deserves a "start with exact pins, revisit if it's actually painful" answer rather than solving it upfront.
