...
func (m *Process) ToORM(ctx context.Context) (ProcessORM, error) {
	...
	for _, v := range m.Metadata {
		if v != nil {
			if tempMetadata, cErr := v.ToORM(ctx); cErr == nil {
				to.Metadata = append(to.Metadata, &tempMetadata)
			} else {
				return to, cErr
			}
		} else {
...
func (m *ProcessORM) ToPB(ctx context.Context) (Process, error) {
	...
	for _, v := range m.Metadata {
		if v != nil {
			if tempMetadata, cErr := v.ToPB(ctx); cErr == nil {
				to.Metadata = append(to.Metadata, &tempMetadata)
			} else {
				return to, cErr
			}
...
func DefaultStrictUpdateProcess(ctx context.Context, in *Process, db *gorm.DB) (*Process, error) {
	...
	filterMetadata.ProcessId = new(string)
	*filterMetadata.ProcessId = ormObj.Id
