...
func (m *Event) ToORM(ctx context.Context) (EventORM, error) {
	...
	to.AggregateType = int32(m.AggregateType)
...
func (m *EventORM) ToPB(ctx context.Context) (Event, error) {
	...
	to.AggregateType = AggregateType(m.AggregateType)
...
func DefaultApplyFieldMaskEvent(ctx context.Context, patchee *Event, patcher *Event, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Event, error) {
	...
	for i, f := range updateMask.Paths {
		...
		if f == prefix+"AggregateType" {
			patchee.AggregateType = patcher.AggregateType
			continue
		}
...
type EventServiceDefaultServer struct {
	DB *gorm.DB
}

// Create ...
func (m *EventServiceDefaultServer) Create(ctx context.Context, in *CreateEventRequest) (*CreateEventResponse, error) {
	db := m.DB
	if custom, ok := interface{}(in).(EventServiceEventWithBeforeCreate); ok {
		var err error
	...
	if custom, ok := interface{}(in).(EventServiceEventWithAfterCreate); ok {
		var err error
...
// EventServiceEventWithBeforeCreate called before DefaultCreateEvent in the default Create handler
type EventServiceEventWithBeforeCreate interface {
	BeforeCreate(context.Context, *gorm.DB) (*gorm.DB, error)
}

// EventServiceEventWithAfterCreate called before DefaultCreateEvent in the default Create handler
type EventServiceEventWithAfterCreate interface {
	AfterCreate(context.Context, *CreateEventResponse, *gorm.DB) error
}

// Read ...
func (m *EventServiceDefaultServer) Read(ctx context.Context, in *ReadEventRequest) (*ReadEventResponse, error) {
	db := m.DB
	if custom, ok := interface{}(in).(EventServiceEventWithBeforeRead); ok {
		var err error
		if db, err = custom.BeforeRead(ctx, db); err != nil {
			return nil, err
		}
	}
	res, err := DefaultReadEvent(ctx, &Event{Id: in.GetId()}, db)
	if err != nil {
		return nil, err
	}
	out := &ReadEventResponse{Result: res}
	if custom, ok := interface{}(in).(EventServiceEventWithAfterRead); ok {
		var err error
...
// EventServiceEventWithBeforeRead called before DefaultReadEvent in the default Read handler
type EventServiceEventWithBeforeRead interface {
	BeforeRead(context.Context, *gorm.DB) (*gorm.DB, error)
}

// EventServiceEventWithAfterRead called before DefaultReadEvent in the default Read handler
type EventServiceEventWithAfterRead interface {
	AfterRead(context.Context, *ReadEventResponse, *gorm.DB) error
}

// Snapshot ...
func (m *EventServiceDefaultServer) Snapshot(ctx context.Context, in *SnapshotRequest) (*SnapshotResponse, error) {
